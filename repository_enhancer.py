        ...
        for package in packages:
            ...
            for external_ref in package['externalRefs']:
                ...
                if reference_type != 'purl':
                    continue
                purl_found = True
                purl = external_ref['referenceLocator']
                # print(f"{package['name']} - {purl}")
                break
            if not purl_found:
                raise ValueError(f"`externalRefs` but missing PURL for {package['name']} in {sbom['documentNamespace']}")
